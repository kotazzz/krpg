...
try:
    ...
    print("Ошибки игры все еще выводятся в старом формате")
...
class GameEventHandler:
    ...
    def listen(self, event: str):
        def wrapper(handler: callable[dict]):
            self.handlers[event].append(handler)

        return wrapper

    def autoregister(self, event: dict):
        if event["type"] in self.types:
            return self.types[event["type"]](**event)
        else:
            event_name = event["type"]
            event_type = make_dataclass(
                event_name,
                {k: type(v) for k, v in event.items()},
                bases=(baseevent,),
            )
            self.types[event_name] = event_type
            return event_type(**event)

    def dispatch(self, event: dict):
        event = self.autoregister(event)

        for base in type(event).__bases__:
            base = base.__name__
            for handler in self.handlers[base]:
                handler(event)
        for handler in self.handlers[type(event).__name__]:
            handler(event)
...
class ReqHandler:
    ...
    def __call__(self, game: Game):
        self._game = game
        self.handler: GameEventHandler = game.handler
        return self
...
class Action:
    def __init__(self, callback, command, description, category=None, time=0):
...
def action(command, description, category=None, time=0):
    def decorator(func):
        return Action(func, command, description, category, time)
...
class ActionManager:
    ...
    def __init__(self) -> None:
        self._submanagers: list[ActionManager] = []

    @property
    def submanagers(self):
        return [s() if callable(s) else s for s in self._submanagers]

    def add_submanagers(self, *sub):
        self._submanagers.extend(sub)

    def actions(self):
        acts = [
            getattr(self, i) for i in dir(self) if isinstance(getattr(self, i), Action)
        ]
        all_actions = {}

        for act in acts:
            category = act.category or self.category
            if category not in all_actions:
                all_actions[category] = []
            all_actions[category].append(act)

        for sub in self.submanagers:
            for category, actions in sub.actions().items():
                if category not in all_actions:
                    all_actions[category] = []
                all_actions[category].extend(actions)
    ...
    def list_actions(self) -> list[Action]:
        ...
        for sub in self.submanagers:
            actions.extend(sub.list_actions())
        return actions

    def __repr__(self):
        return f"<ActionManager {self.__class__.__name__}>"


class Scenario(ReqHandler):
    def __init__(self, identifier, state, scenario):
    ...
    def remember(self):
        self.breakpoints.append((self.state, self.cursor))

    def restore(self):
        self.state, self.cursor = self.breakpoints.pop()

    def goto(self, new_state):
        old_branch, old_cursor = self.branch, self.cursor
        self.state = new_state
        self.run()
        return old_branch, old_cursor

    def cast(self, *text, min=0.02):
        data = " ".join(map(str, text))
        # replace $var with self.env["var"]
        for var in re.findall(r"\$[a-zA-Z_][a-zA-Z0-9_]*", data):
            data = data.replace(var, str(self.env.get(var[1:], "")))

        # replace $(expr) with eval
        for expr in re.findall(r"\$\([^)]*\)", data):
            data = data.replace(expr, str(eval(expr[2:-1], globals(), self.env)))

        self._game.console.print(data, min=min)

    def wait(self):
        self._game.console.wait("Нажмите для продолжения")

    def say(self, who, *args, color=None):
        # self._game.console.print(f"[bold {self.color}]{who}[/]: ", end="")
        # self._game.console.print(" ".join(map(str, args)), min=0.02)
        color = f"[bold {color}]" if color else ""
        self.cast(f"{color}{who}[/]:", *args, min=0.02)
        self.wait()

    def execute(self, command, *args, orig=""):
        game = self._game
        player = game.player
        ...
        if command == "sayb":
            self.say(game.player.name, *args, color="cyan")
        elif command == "sayc":
            name = args[0]
            self.say(name, *args[1:])
        elif command == "saye":
            npc_id = args[0]
            npc = game.bestiary.get_npc(npc_id)
            npc.say(npc.name, *args[1:], color=npc.color)
        elif command == "cast":
            self.cast(f'[bold bwhite]{" ".join(map(str, args))}[/]', min=0.005)
            self.wait()

        elif command == "e":
            npc_id = args[0]
            npc = game.bestiary.get_npc(npc_id)
            npc.execute(*args[1:], orig=orig)

        elif command == "give_quest":
            game.quest_manager.give(args[0])
        elif command == "take":
            game.quest_manager.take(args[0])
        elif command == "give":
            id, count = args[0], int(args[1])
            player.inventory.pickup(game.bestiary, id, count)
            ...
            if title:
                self.say(title, "yellow")
            for i, opt in enumerate(opts, 1):
                self.cast(f"  [green]{i}[/] - {opt}")
            sel = game.console.number_menu(2, len(opts))
            new_branch_key = list(opts.keys())[sel]
            self.say(game.player.name, new_branch_key)
            new_branch = opts[new_branch_key]
            self.goto(new_branch)
            self.restore()

        elif command == "menu_exhaust":
            self.remember()
            title = args[0]
            opts = dict(zip(args[1::2], args[2::2]))
            if title:
            ...
            while opts:
                ...
                new_branch = opts[new_branch_key]
                self.say(game.player.name, new_branch_key)
                self.scenario[self.state], self.cursor
                self.goto(new_branch)

                opts.pop(new_branch_key)
            self.restore()
            ...
            self.cursor = int(args[0])

        elif command == "set":
            varname, expr = args[:2]
            self.env[varname] = eval(expr, globals(), self.env)
        elif command == "cases":
            # expr1 state1 expr2 state2 -> [expr1, state1], [expr2, state2]
            cases = list(zip(args[::2], args[1::2]))
            for expr, state in cases:
                if eval(expr, globals(), self.env):
                    self.goto(state)
                    break
        elif command == "exec":
            exec(orig[5:], globals(), self.env)

        elif command == "sell":
            # show all items in inventory
            last = None
            for slot in inventory.storage_slots:
                if slot.itemtype != last:
                    console.print(
                        f"[bold green]{ItemType.description(slot.itemtype)}[/]: ",
                        end="",
                    )
                    last = slot.itemtype
                if not slot.empty:
                    item = game.bestiary.get_item(slot.itemid)
                    console.print(f"[white]{slot.count}x[green]{item.name}[/] ", end="")
                else:
                    console.print("[yellow]-[/] ", end="")
            console.print()
            if any(not slot.empty for slot in inventory.storage_slots):
                while True:
                    console.print("[bold green]Выберите слот для продажи (e-выход)[/]")
                    sel = console.number_menu(1, len(inventory.storage_slots), "e")
                    if sel == "e":
                        break
                    slot = inventory.storage_slots[sel]
                    if not slot.empty:
                        item: Item = game.bestiary.get_item(slot.itemid)
                        if item.sellable:
                            console.print(
                                f"[bold green]Вы продали [white]{slot.count}x[cyan]{item.name} [yellow]+{slot.count * item.sell}[/]"
                            )
                            game.player.add_money(slot.count * item.sell)
                            slot.count = 0
                        else:
                            console.print(
                                f"[bold yellow]Вы не можете продать {item.name}[/]"
                            )
                    else:
                        console.print("[bold yellow]Этот слот пуст[/]")
        elif command == "buy":
            # args - list of item ids
            for i, item_id in enumerate(args, 1):
                item = game.bestiary.get_item(item_id)
                # # name price
                console.print(
                    f"[bold green]{i}[/] - [cyan]{item.name}[/] [yellow]{item.buy}[/]"
                )
            while True:
                console.print("[bold green]Выберите предмет для покупки (e-выход)[/]")
                sel = console.number_menu(2, len(args), "e")
                if sel == "e":
                    break
                item = game.bestiary.get_item(args[sel])
                if player.money >= item.buy:
                    player.add_money(-item.buy)
                    player.inventory.pickup(game.bestiary, item.id, 1)
                    console.print(f"[bold green]Вы купили [cyan]{item.name}[/]")
                else:
                    console.print("[bold yellow]У вас недостаточно денег[/]")
        elif command == "save":
            code = game.save()
            console.print(f"[bold green]Ваш код сохранения:\n[bold blue]{code}[/]")
        elif command == "load":
            console.print("[bold green]Введите код сохранения[/]")
            code = input()
            game.load(code)
        elif command == "upgrade":
            p = game.player
            console.print(
                "[green]Добро пожаловать в мастер распределения характеристик!"
            )
            console.print(f"Вам доступно [bold white]{p.free_points}[/] очков.")
            console.print(f"Сила ([red]S[/]): [bold red]{p.strength}[/]")
            console.print(f"Ловкость ([red]D[/]): [bold red]{p.dexterity}[/]")
            console.print(f"Мудрость ([red]W[/]): [bold red]{p.wisdom}[/]")
            console.print(f"Выносливость ([red]E[/]): [bold red]{p.endurance}[/]")
            console.print("Введите [red]exit[/] для завершения распределения.")
            console.print("Введите характеристику, которую хотите повысить.")
            while p.free_points > 0:
                char = console.prompt(2)
                if char == "exit":
                    break
                if char == "S":
                    p.properties.base_strength += 1
                elif char == "D":
                    p.properties.base_dexterity += 1
                elif char == "W":
                    p.properties.base_wisdom += 1
                elif char == "E":
                    p.properties.base_endurance += 1
                else:
                    console.print(
                        "[red]Неверная характеристика. Введите S, D, W или E.[/]"
                    )
                    continue
                p.free_points -= 1
            console.print("[green]Распределение завершено.[/]")

    @property
    def branch(self):
        return self.scenario[self.state]

    def run(self, cursor=0):
        self.cursor = cursor
        self.running = True
        while True:
            line = self.branch[self.cursor]
            command, *args = shlex.split(line)
            self.execute(command, *args, orig=line)
            self.cursor += 1
            if self.cursor >= len(self.branch) or not self.running:
                break

    def __repr__(self):
        return f"<Scenario {self.identifier} {self.state} {self.cursor}>"


class NotPlayableCharacter(Scenario):
    def __init__(
        self, name, description, state, identifier, scenario, known=False, color="blue"
    ):
        self.name = name
        self.description = description
        self.known = known
        ...
        Scenario.__init__(self, identifier, state, scenario)
    ...
    def execute(self, command, *args, orig=""):
        ...
        if command == "say":
            self.say(self.name if self.known else "???", *args, color=self.color)
        elif command == "known":
            self.known = False if args else True
        else:
            super().execute(command, *args, orig=orig)

    def __repr__(self):
        return f"<NotPlayableCharacter {self.identifier} {self.state} {self.cursor}>"


class Location(ActionManager, ReqHandler):
    name = "Локация"
    ...
    def save(self):
        return [
            self.drop,
            {n.identifier: n.save() for n in self.npcs},
        ]
    ...
    def __init__(self):
        ...
        # id, count
        self.drop = []
        self.npcs: list[NotPlayableCharacter] = []
        self.category = self.name
        super().__init__()

    def add_drop(self, itemid, count):
        self.drop.append((itemid, count))
        return self

    def add_npc(self, npc):
        npc = self._game.bestiary.get_npc(npc)
        self.npcs.append(npc)
        return self

    def link(self, location, hide=False):
        if hide:
            self.hided.append(location)
            location.hided.append(self)
            return self
        else:
            self.sublocations.append(location)
            location.sublocations.append(self)
            return self

    def unlink(self, location):
        if location in self.sublocations:
            self.sublocations.remove(location)
            location.sublocations.remove(self)

    def hide(self, location):
        self.unlink(location)
        self.link(location, True)

    def find(self, location):
        self.unlink(location)
        self.link(location, False)

    def __repr__(self):
        return f"<Location {self.name}>"


class ScenarioLocation(Location):
    def __init__(
        self, identifier, name, description, actions: dict[(str, str, int), Scenario]
    ):
        super().__init__()
        self.identifier = identifier
        self.name = name
        self.description = description
        for (name, desc, time), scenario in actions.items():
            self.gen_action(name, desc, time, scenario)

    def gen_action(self, name, desc, time, scenario):
        def cb(game: Game):
            scenario.run()

        setattr(self, f"command_{name}", Action(cb, name, desc, time=time))
...
class ScriptParser:
    ...
    def scenario(self, scenario):
        result = {}
        cursor = []
        scenario = self.get_lines(scenario)
        for line in scenario:
            if line.startswith(":"):
                ...
                if cursor[-1] not in result:
                    result[cursor[-1]] = [line]
                else:
                    result[cursor[-1]].append(line)
        return result

    def full_load(self, game, scenario):
        data = self.parse(scenario)
        self.load(game, data)
    ...
    def parse(self, scenario):
        ...
        for line in scenario:
            if line.startswith("@"):
                if line.startswith("@location"):
                    ...
                    action = dict(
                        zip(
                            ("command", "description", "time", "identifier", "state"),
                            shlex.split(line[8:]),
                        )
                    )
                    action["scenario"] = ""
                    locations[-1]["actions"].append(action)
                elif line.startswith("@map"):
                    stack.append("map")
                # quest "first_quest" "Зайти в лес" "Вы должны найти своих 'спасителей'"
                elif line.startswith("@quest"):
                    stack.append("quest")
                    quest = dict(
                        zip(
                            ("identifier", "name", "description"), shlex.split(line[7:])
                        )
                    )
                    quest["requirements"] = []
                    quest["reward"] = []
                ...
                # @entities
                    ...
                    stack.append("entities")
                # name desc state identifier known [color] [scenario]
                # @npc "Торговец" "Продает и покупает интересные вещи" "none" "npc" True blue
                ...
                # scenario "game" "none"
                    ...
                    scenario = dict(
                        zip(("identifier", "state"), shlex.split(line[10:]))
                    )
                    scenario["scenario"] = ""
                ...
                if stack[-1] == "location":
                    ...
                    if cmd == "drop":
                        locations[-1]["drop"].append(args)
                    elif cmd == "npc":
                        locations[-1]["npc"].append(args[0])
                elif stack[-1] == "action":
                    locations[-1]["actions"][-1]["scenario"] += line + "\n"
                elif stack[-1] == "map":
                    cmd, *args = shlex.split(line)
                    if cmd == "link":
                        links.append(args)
                    ...
                    cmd, *args = shlex.split(line)
                    if cmd == "req":
                        quests[-1]["requirements"].append(args[0])
                    elif cmd == "rew":
                        quests[-1]["reward"].append(args[0])
                # add "Безумная крыса" 1 1 1 1 0 25
                elif stack[-1] == "entities":
                    cmd, *args = shlex.split(line)
                    if cmd == "add":
                        entities.append(args)
                elif stack[-1] == "npc":
                    npcs[-1]["scenario"] += line + "\n"
                elif stack[-1] == "scenario":
                    scenarios[-1]["scenario"] += line + "\n"
                elif stack[-1] == "items":
                    cmd, *args = shlex.split(line)
                    if cmd == "item":
                        item = {"base": args}
        ...
        return locations, quests, entities, npcs, scenarios, items, links, root, default

    def load(self, game: Game, alldata):
        (
            locations,
            quests,
            entities,
        ...
        game.quest_manager.load_quests(*[Quest(**q) for q in quests])
        # game.bestiary.load_entities(*[EntityInfo(*e) for e in entities])
        new_entities = []
        for e in entities:
            e[1:] = map(int, e[1:])
            new_entities.append(EntityInfo(*e))
        game.bestiary.load_entities(*new_entities)

        new_npcs = []
        for n in npcs:
            n["scenario"] = self.scenario(n["scenario"])
            new_npcs.append(NotPlayableCharacter(**n)(game))
        game.bestiary.load_npcs(*new_npcs)
        new_scenarios = []
        for s in scenarios:
            s["scenario"] = self.scenario(s["scenario"])
            new_scenarios.append(Scenario(**s)(game))
        game.bestiary.load_scenarios(*new_scenarios)
        new_items = []
        for i in items:
            i["base"][3] = int(i["base"][3])
            item = Item(*i["base"])
            if "use" in i:
                item.setup_use(*map(int, i["use"]))
            if "price" in i:
                item.setup_price(*map(int, i["price"]))
            if "wear" in i:
                wear_type = ItemType.__members__[i["wear"].pop(-1)]
                params = map(int, i["wear"])
                item.setup_wear(*params, wear_type)
            new_items.append(item)
        game.bestiary.load_items(*new_items)
        ...
        for data in locations:
            ...
            for drop in data["drop"]:
                loc.add_drop(drop[0], int(drop[1]))
            for npc in data["npc"]:
                loc.add_npc(npc)
            for action in data["actions"]:
                loc.gen_action(
                    ...
                    scenario=Scenario(
                        ...
                        state=action["state"],
                        scenario=self.scenario(action["scenario"]),
                    )(game),
                )
        ...
        for a, b in links:
            get_location(a).link(get_location(b))

        game.location = get_location(default)
        game.world.set_root(get_location(root))


@dataclass
class EntityInfo:
    # name s d w e free money
    name: str
    s: int
    d: int
    w: int
...
class Properties:
    def __init__(self, s, d, w, e, start_boost=None):
        self.base_strength = s  # Сила
        self.base_dexterity = d  # Ловкость
        self.base_wisdom = w  # Мудрость
        self.base_endurance = e  # Выносливость
        self.boost = start_boost or []

        self.hp = self.max_hp
        self.mp = self.max_mp

    def set_boost(self, boost):
        self.boost: list[PropertyUpdate] = boost

    @classmethod
    def from_entityinfo(cls, info: EntityInfo):
        return cls(info.s, info.d, info.w, info.e)

    @property
    def strength(self):
        return self.base_strength + sum(map(lambda i: i.s, self.boost))

    @property
    def dexterity(self):
        return self.base_dexterity + sum(map(lambda i: i.d, self.boost))

    @property
    def wisdom(self):
        return self.base_wisdom + sum(map(lambda i: i.w, self.boost))

    @property
    def endurance(self):
        return self.base_endurance + sum(map(lambda i: i.e, self.boost))

    @property
    def attack(self):
        return self.strength * 2 + 1 + sum(map(lambda i: i.atk, self.boost))

    @property
    def defense(self):
        return self.dexterity * 2 + 0.5 + sum(map(lambda i: i.defense, self.boost))

    @property
    def max_mp(self):
        return self.wisdom * 10 + 5 + sum(map(lambda i: i.mhp, self.boost))

    @property
    def max_hp(self):
        return self.endurance * 10 + 10 + sum(map(lambda i: i.mmp, self.boost))


class Entity(ReqHandler):
    def __init__(
        self,
        bestiary: Bestiary,
        ...
        self.properties = Properties.from_entityinfo(entityinfo)
        self.free_points = entityinfo.free
        self.money = entityinfo.money
        self.hp = self.max_hp
        self.mp = self.max_mp

    def __call__(self, game):
        v = super().__call__(game)
        self.inventory = Inventory(on_change=self.on_change)(self._game)
        self.properties.set_boost(self.boost())
        return v
    ...
    def add_sdwe(self, s=0, d=0, w=0, e=0):
        self.properties.base_strength += s
        self.properties.base_dexterity += d
        self.properties.base_wisdom += w
        self.properties.base_endurance += e
    ...
    def load(self, data):
        self.name = data[0]
        self.properties.base_strength = data[1]
        self.properties.base_dexterity = data[2]
        self.properties.base_wisdom = data[3]
        self.properties.base_endurance = data[4]
    ...
    def boost(self):
        return self.inventory.full_boost(self.bestiary)
    ...
    @property
    def dexterity(self):
        return self.properties.dexterity

    @property
    def wisdom(self):
        return self.properties.wisdom

    @property
    def endurance(self):
        return self.properties.endurance


class Bestiary(ReqHandler):
    def __init__(self):
    ...
    def load_entities(self, *entities: list[EntityInfo]):
        self.entities.extend(entities)

    def load_items(self, *items: list[Item]):
        self.items.extend(items)

    def load_npcs(self, *npcs: list[NotPlayableCharacter]):
    ...
    def add_npc(self, npc: NotPlayableCharacter):
        self.npcs.append(npc)
    ...
    def get_entity(self, id):
        if id < len(self.entities):
            return Entity(self, self.entities[id])(self._game)

    def get_random_entity(self) -> Entity:
        return Entity(self, random.choice(self.entities))(self._game)

    def get_item(self, itemid: str) -> Item | None:
        for item in self.items:
            if item.id == itemid:
                return item

    def get_npc(self, identifier: str) -> NotPlayableCharacter | None:
    ...
    def wait(self, minutes):
        ...
        if self.days > day:
            self.handler.dispatch(
                {"type": "newday", "game": self._game, "day": self.days, "clock": self}
            )
...
class BattleManager(ReqHandler):
    def __init__(self) -> None:
        pass

    def new(self, entity: Entity):
        """
        Игрок выбирает: атака, защита, побег
        Противник выбирает: атака, защита
        Если а-а: оба получают урон
        если а-з/з-а: атакуемый получает часть урона
        если з-з: никто ничего не получает
        """
        game = self._game
        player = game.player

        game.console.print(f"[bold red]Вы встретили {entity.name}[/]")
        game.console.presense(entity)
        game.console.print("Выбор действия:")
        game.console.print("[red] 1 [/] Атака")
        game.console.print("[blue] 2 [/] Защита")
        game.console.print("[yellow] 3 [/] Попытка побега (40%)")
        while True:
            pselect = game.console.number_menu(2, 3) + 1
            pselect = (
                (4 if random.random() < 0.4 else 3) if pselect == "3" else int(pselect)
            )
            ...
            text = {
                (
                    1,
                    1,
                ): "Вы атаковали друг друга. [bold]{en}[/] получает [red]{pa:.2f}[/] урона, вы получаете [red]{ea:.2f}[/] урона.",
                (
                    1,
                    2,
                ): "Вы атаковали [bold]{en}[/], который попытался увернуться от вашего удара. [bold]{en}[/] получает [red]{pa:.2f}[/]([blue]-{ed:.2f}[/]) урона.",
                (
                    2,
                    1,
                ): "[bold]{en}[/] решил атаковать, вы решили обороняться. Вы получаете [red]{ea:.2f}[/]([blue]-{pd:.2f}[/]) урона.",
                (2, 2): "Вы попытались избежать атаки, никто ничего не получил",
                (
                    3,
                    1,
                ): "Вы попытались сбежать, но не получилось, [bold]{en}[/] нанес вам [red]{pa:.2f}[/] урона",
                (
                    3,
                    2,
                ): "Вы попытались сбежать, но не получилось, [bold]{en}[/] решил не атаковать вас",
                (4, 1): "Вы сбежали, [bold]{en}[/] не смог нанести вам урона",
                (4, 2): "Вы сбежали, [bold]{en}[/] решил не атаковать вас",
            }
            # e - entity, p - player
            # n - name, a - урон, наносимый сущность, d - поглащенный урон, r - результатный урон
            pn = player.name
            pa = player.attack * random.uniform(0.8, 1.2) * (pselect == 1)
            pd = player.defense * random.uniform(0.8, 1.2) * (pselect == 2)
            en = entity.name
            ea = entity.attack * random.uniform(0.8, 1.2) * (eselect == 1)
            ed = entity.defense * random.uniform(0.8, 1.2) * (eselect == 2)
            pr = max(pa - ed, 0)
            er = max(ea - pd, 0)
            player.hp = max(player.hp - er, 0)
            entity.hp = max(entity.hp - pr, 0)

            game.console.print(
                text[(pselect, eselect)].format(
                    pn=pn,
                    pa=pa,
                    pd=pd,
                    en=en,
            ...
            game.console.presenses(player, entity)

            if player.hp <= 0:
                game.state = State.DIED
                return False
            elif entity.hp <= 0 or pselect == 4:
                self.handler.dispatch(
                    {
                        "type": "entity_killed",
                        "entity": entity,
...
class ItemType(Enum):
    ...
    # ItemType.description[ItemType.HELMET] == "Шлем"


class Slot:
    def __init__(self, itemtype, itemid, count):
        self.itemtype = itemtype
        self.itemid = itemid
        self.count = count

    def swap(self, slot: Slot):
        self.itemid, slot.itemid = slot.itemid, self.itemid
        self.count, slot.count = slot.count, self.count

    @property
    def empty(self):
        return self.count == 0 or self.itemid == -1

    def __repr__(self) -> str:
        return f"<Slot {self.itemtype.name} {self.itemid} {self.count}>"


class PropertyUpdate:
    def __init__(self, name, s, d, w, e, atk, defense, mhp, mmp):
        # s, d, w, e, atk, defense, mhp, mmp
        self.name = name
        self.s = s
        self.d = d
        self.w = w
    ...
    def __repr__(self) -> str:
        return f"<PropertyUpdate {self.name!r}>"


class Inventory(ReqHandler):
    def save(self):
        return [(slot.itemid, slot.count) for slot in self.slots]

    def load(self, data):
        for slot, (i, c) in zip(self.slots, data):
            slot.itemid = i
            slot.count = c

    def __init__(self, on_change=None):
        # 10 предметов, 1 шлем, 1 нагрудник, 1 поножи, 1 ботинки, 1 перчатки, 1 щит, 1 оружие, 2 кольца, 1 амулет
        self.wear_slot_start = 10
        self.slots = [Slot(ItemType.ITEM, -1, 0) for _ in range(10)] + [
            Slot(ItemType.HELMET, -1, 0),
            Slot(ItemType.CHESTPLATE, -1, 0),
            Slot(ItemType.LEGGINGS, -1, 0),
            Slot(ItemType.BOOTS, -1, 0),
            Slot(ItemType.GLOVES, -1, 0),
            Slot(ItemType.SHIELD, -1, 0),
            Slot(ItemType.WEAPON, -1, 0),
            Slot(ItemType.RING, -1, 0),
            Slot(ItemType.RING, -1, 0),
            Slot(ItemType.AMULET, -1, 0),
        ]
        self.items = []
    ...
    @property
    def wear_slots(self):
        return self.slots[self.wear_slot_start :]

    @property
    def storage_slots(self):
        return self.slots[: self.wear_slot_start]

    def full_boost(self, bestiary):
        updates = []
        for slot in self.wear_slots:
            if not slot.empty:
                item = bestiary.get_item(slot.itemid)
                update = PropertyUpdate(
                    f"Эффект от '{item.name}'",
                    item.strength,
                    item.dexterity,
                    item.wisdom,
                    item.endurance,
    ...
    def pickup(self, bestiary, item_id, count):
        # pickup inly in ITEM slot
        item = bestiary.get_item(item_id)
        for slot in self.slots[:10]:
            ...
            if slot.itemid == item_id:
                # if item already in inventory and it smaller, than item.max_stack
                new_count = slot.count + count
                slot.count = min(new_count, item.max_stack)
                count = max(new_count - item.max_stack, 0)
    ...
    def wear(self, bestiary, slot_id):
        ...
        if item.itemtype != ItemType.ITEM:
            for slot_item in self.slots:
                if slot_item.itemtype == item.itemtype:
                    slot_item.swap(slot)
                    self.on_change()
                    return True
        return False
...
class Item:
    def __init__(self, id, name, description, max_stack):
        self.id = id
        self.name = name
        self.description = description
        ...
        self.usable = False
        self.sellable = False
        self.itemtype = ItemType.ITEM

    def setup_wear(self, s, d, w, e, a, de, mhp, mmp, newtype):
        self.strength = s
        self.dexterity = d
        self.wisdom = w
        self.endurance = e
    ...
    def setup_use(self, s, d, w, e, hp, mp, free, money):
        self.usable = True
        self.strength = s
        self.dexterity = d
        self.wisdom = w
...
class Settings:
    ...
    def load(self, data):
        for key, value in zip(self.data.keys(), data):
            self.data[key] = value
...
class BaseManager(ActionManager):
    category = "Игра"

    @action("exit", "Выйти из игры")
    def command_exit(game: Game):
        game.state = State.EXIT
        game.console.print("[bold red]Выход из игры...")

    @action("help", "Показать список команд", "Помощь")
    def command_help(game: Game):
        for category, actions in game.manager.actions().items():
            game.console.print(f"[bold white]{category}[/]")
            for action in actions:
                game.console.print(
                    f"  [green]{action.command}[/] - {action.description}"
                )
            game.console.print()

    @action("me", "Показать информацию о персонаже", "Информация")
    def command_me(game: Game):
        inventory = game.player.inventory
        console = game.console

        console.print(f"[green]Ваш баланс:   [yellow]{game.player.money}[/]")
        console.print(f"[green]Ваша локация: [blue]{game.location.name}[/]")
        game.console.presense(
            game.player,
        )
        ...
        for slot in inventory.slots:
            if not slot.empty:
                ...
                if slot.itemtype != last:
                    console.print(
                        f"\n[bold green]{ItemType.description(slot.itemtype)}[/]: ",
                        end="",
                    )
                    last = slot.itemtype
                ...
                console.print(f"[white]{slot.count}x[green]{item.name}[/] ", end="")
        if not equiped:
            console.print("[bold red]Нет экипировки[/]")
        else:
            console.print()

        if not (boost := game.player.boost()):
            console.print("[bold red]Нет модификаторов[/]")
        else:
            console.print(
                f"[bold white]{'Модификаторы':<30}[bold red]{'+s':<3}[bold blue]{'+d':<3}"
                f"[bold yellow]{'+w':<3}[bold green]{'+e':<3}[bold red]{'+atk':<5}"
                f"[bold blue]{'+def':<5}[bold green]{'+hp':<5}[bold cyan]{'+mp':<5}[/]"
            )
            for mod in boost:
                console.print(
                    f"[white]{mod.name:<30}[red]{mod.s:<3}[blue]{mod.d:<3}"
                    f"[yellow]{mod.w:<3}[green]{mod.e:<3}[red]{mod.atk:<5}"
                    f"[blue]{mod.defense:<5}[green]{mod.mhp:<5}[cyan]{mod.mmp:<5}[/]"
                )

    @action("time", "Сколько времени", "Информация")
    def command_time(game: Game):
        c = game.clock
        game.console.print(f"Время: День {c.days}, {c.hours:0>2}:{c.minutes:0>2}")

    @action("look", "Осмотреть локацию", "Информация", time=5)
    def command_look(game: Game):
        ...
        if not location.drop:
            game.console.print("  [bold red]Ничего интересного[/]")
        for drop in location.drop:
            item = game.bestiary.get_item(drop[0])
            game.console.print(f"  [white]{drop[1]}x[green]{item.name}[/]")
        game.console.print()

        game.console.print("[bold green]Персонажи[/]")
        if not location.npcs:
            game.console.print("  [bold red]Никого нет[/]")
        for npc in location.npcs:
            game.console.print(f"  [bold white]{npc.name}[/] - {npc.description}")
        game.console.print()

    @action("talk", "Поговорить с персонажем", "Действия", time=5)
    def command_talk(game: Game):
        location = game.location
        game.console.print("[bold green]Персонажи[/]")
        if not location.npcs:
            game.console.print("  [bold red]Никого нет[/]")
        for i, npc in enumerate(location.npcs, 1):
            game.console.print(
                f"  [green]{i}[/] - [bold white]{npc.name}[/] - {npc.description}"
            )
        game.console.print("  [bold red]e[/] - Выйти")

        sel = game.console.number_menu(2, len(location.npcs), "e")
        if sel == "e":
            return
        npc = location.npcs[sel]
        npc.run()

    @action("go", "Пойти в другую локацию", "Действия", time=10)
    def command_go(game: Game):
        location = game.location
        game.console.print(
            f"[bold green]Ваша позиция: [bold white]{location.name} - {location.description}\nДоступно:[/][/]"
        )
        if not location.sublocations:
            game.console.print("  [bold red]Нет доступных направлений[/]")
        for i, sub in enumerate(location.sublocations, 1):
            game.console.print(f"  [green]{i}[/] - [bold blue]{sub.name}[/]")
        game.console.print("  [bold red]e[/] - Выйти")

        sel = game.console.number_menu(2, len(location.sublocations), "e")
        if sel == "e":
            return

        game.handler.dispatch(
            {
                "type": "location_move",
                "next": game.location.sublocations[sel],
                "prev": game.location,
                "game": game,
            }
        )
        game.location = game.location.sublocations[sel]

    @action("pickup", "Подобрать предмет", "Действия", time=1)
    def command_pickup(game: Game):
        ...
        if not location.drop:
            game.console.print("  [bold red]Ничего интересного[/]")
        for i, drop in enumerate(location.drop, 1):
            item = game.bestiary.get_item(drop[0])
            game.console.print(
                f"  [green]{i}[/] - [white]{drop[1]}x[green]{item.name}[/]"
            )
        game.console.print("  [bold red]e[/] - Выйти")

        sel = game.console.number_menu(2, len(location.drop), "e")
        if sel == "e":
            return
        remain = game.player.inventory.pickup(game.bestiary, *location.drop[sel])
        if remain:
            location.drop[sel] = (location.drop[sel][0], remain)
        else:
            location.drop.pop(sel)

    @action("guide", "Гайды и справка", "Помощь")
    def command_guide(game: Game):
        passages = {
            "FAQ": ("Тут пусто :("),
            "Об игре": (
                "[green]Автор:[/] Kotaz\n"
                "[green]Язык:[/] Python 3\n"
                "[green]Библиотеки:[/] Упс, а игра без зависимостей...\n"
                "[green]Кол-во строк кода:[/] 3000+\n\n"
                "[bold green]Отдельная благодарность:[/]\n"
                "  [green]Никто[/]\n"
                "  [red]Конец[/]\n"
            ),
            "SDWE система": (
                "[green]S[/] - strength - сила\n"
                "[green]D[/] - dexterity - ловкость\n"
                "[green]W[/] - wisdom - мудрость\n"
                "[green]E[/] - endurance - выносливость\n"
                "[green]F[/] - free - свободные очки \n\n"
                "[green]A/ATK[/] - attack - атака\n"
                "[green]D/DEF[/] - defense - защита\n"
                "[green]HP[/] - health (points) - здоровье\n"
                "[green]MP[/] - mana (points) - мана\n"
            ),
            ...
            "Макросы": (
                "[green]Макросы[/] - это команды, которые можно "
                "вызывать вне зависимости от состояния игры. "
                "Они не влияют на игру, но влияют на взаимодействие с игрой\n"
                "[green].?[/] - вызов справки\n"
            ),
        ...
        for i, passage in enumerate(passages, 1):
            game.console.print(f"[green]{i}[/] - {passage}")
        while True:
            game.console.print("[green]Выберите секцию (e - Выход)[/]")
            select = game.console.number_menu(2, len(passages), "e")
            if select == "e":
                return
            else:
                text = wrap(list(passages.values())[select])
                game.console.print("\n".join(text))

    @action("bestiary", "Посмотреть бестиарий", "Помощь")
    def command_bestiary(game: Game):
        game.console.print("[bold green]Бестиарий[/]")
        # get input from user, variants: n, s, d, w, e, f, m, 0, !
        game.console.print("[bold green]Варианты сортировки:[/]")
        game.console.print(
            "[green]n[/] - По имени [green]s[/] - По силе\n"
            "[green]d[/] - По ловкости [green]w[/] - По мудрости\n"
            "[green]e[/] - По выносливости [green]f[/] - По кол-ву свободных очков\n"
            "[green]m[/] - По кол-ву монет [green]0[/] - По умолчанию\n"
            "[yellow]-[/] - В обратном порядке (добавьте в конец)\n"
            "[red]![/] - Выход"
        )

        usort = game.console.prompt(
            2,
            lambda x: x == "!"
            or x[0] in "nsdefm"
            and (len(x) == 2 and x[1] == "-")
        ...
        if usort == "!":
            ...
            sorted_bestiary = game.bestiary.entities
        else:
            variants = {
                "n": "name",
                "s": "s",
            ...
            sorted_bestiary = sorted(
                game.bestiary.entities,
                key=lambda x: asdict(x)[variants[usort[0]]],
                reverse=len(usort) - 2,
            )
        game.console.print(
            f"[bold green]Количество элементов на странице (1...{len(sorted_bestiary)}):[/]"
        )
        page_size = game.console.number_menu(2, len(sorted_bestiary))
        page_count = len(sorted_bestiary) // page_size
        # +1 for index>page, +1 for floor
        game.console.print(f"[bold green]Страница (1...{page_count+1}):[/]")
        page_num = game.console.number_menu(2, page_count + 1)
        sorted_bestiary = sorted_bestiary[
            (page_num - 1) * page_size : page_num * page_size
        ]
        game.console.print(
            f"[green]{'Имя':<15}[/] [bold yellow]+{'$':<5}[/] "
            f"[bold red]S={'s':<5} [bold blue]D={'d':<5} "
            f"[bold yellow]W={'w':<5} [bold green]E={'e':<5} "
            f"[bold white]F={'free':<5}[/]"
        )
        for name, s, d, w, e, free, money in map(
            lambda x: asdict(x).values(), sorted_bestiary
        ):
            game.console.print(
                f"[green]{name:<15}[/] [bold yellow]+{money:<5}[/] "
                f"[bold red]S={s:<5} [bold blue]D={d:<5} "
                f"[bold yellow]W={w:<5} [bold green]E={e:<5} "
                f"[bold white]F={free:<5}[/]"
            )

    @action("inventory", "Управление инвентарем", "Действия")
    def command_inventory(game: Game):
        inventory = game.player.inventory
        console = game.console

        console.print(
            "[bold green]Управление инвентарем[/]\n"
            "  [green]v[white] - просмотр инвентаря[/]\n"
            "  [green]w[white] - надеть предмет[/]\n"
        ...
        while True:
            select = console.prompt(2, checker=lambda x: x in "vwuide")
            if select == "e":
                break
            elif select == "v":
                console.print("[bold green]Инвентарь[/]")
                last = None
                for use_slot in inventory.slots:
                    if use_slot.itemtype != last:
                        console.print(
                            f"\n[bold green]{ItemType.description(use_slot.itemtype)}[/]: ",
                            end="",
                        )
                        last = use_slot.itemtype
                    if use_slot.empty:
                        console.print("[yellow]-[/] ", end="")
                    else:
                        item = game.bestiary.get_item(use_slot.itemid)
                        console.print(
                            f"[white]{use_slot.count}x[green]{item.name}[/] ", end=""
                        )
                console.print()
            elif select == "w":
                console.print("[bold green]Выберите слот сумки (1-10)[/]")
                use_id = console.number_menu(2, 10)

                if inventory.slots[use_id].empty:
                    console.print(
                        "[bold green]Слот пуст, выберите слот снаряжение, из которого хотите забрать предмет\n"
                        "Нумерация идет слева направо, сверху вниз (1-10)[/]\n"
                    )
                    wear_id = console.number_menu(2, 10)
                    wear_slot = inventory.slots[wear_id + 10]
                    use_slot = inventory.slots[use_id]
                    wear_slot.swap(use_slot)
                else:
                    if inventory.wear(game.bestiary, int(use_id)):
                        console.print("[bold green]Предмет надет[/]")
                    else:
                        console.print("[bold red]Невозможно надеть[/]")
            elif select == "u":
                console.print("[bold green]Выберите слот (1-10)[/]")
                use_id = console.number_menu(2, 10)
                use_slot = inventory.slots[int(use_id)]
                player = game.player
                if use_slot.empty:
                    console.print("[bold red]Слот пуст[/]")
                else:
                    item = game.bestiary.get_item(use_slot.itemid)
                    if item.usable:
                        player.add_sdwe(
                            item.strength, item.dexterity, item.wisdom, item.endurance
                        )
                        player.heal(item.hp, item.mp)
                        player.add_money(item.money)
                        player.free_points += item.free
                        use_slot.count -= 1
                        console.print("[bold green]Успех[/]")
                    else:
                        console.print("[bold red]Нельзя использовать[/]")
            elif select == "i":
                console.print("[bold green]Выберите слот(1-10)[/]")
                use_id = console.number_menu(2, 10)
                use_slot = inventory.slots[use_id]
                if use_slot.empty:
                    ...
                    if item.usable:
                        console.print(
                            f"[yellow]Используемый [bold green]{item.name}[/]\n"
                            f"  [blue]{item.description}[/]\n"
                            f"  [white]Сила: [cyan]{item.strength}[/]\n"
                            f"  [white]Ловкость: [cyan]{item.dexterity}[/]\n"
                            f"  [white]Мудрость: [cyan]{item.wisdom}[/]\n"
                            f"  [white]Выносливость: [cyan]{item.endurance}[/]\n"
                            f"  [white]Здоровье: [cyan]{item.hp}[/]\n"
                            f"  [white]Мана: [cyan]{item.mp}[/]\n"
                            f"  [white]Очков: [cyan]{item.free}[/]"
                            f"  [white]Деньги: [cyan]{item.money}[/]"
                        )

                    elif item.itemtype != ItemType.ITEM:
                        console.print(
                            f"[yellow]Надеваемый [bold green]{item.name}[/]\n"
                            f"  [blue]{item.description}[/]\n"
                            f"  [white]Сила: [cyan]{item.strength}[/]\n"
                            f"  [white]Ловкость: [cyan]{item.dexterity}[/]\n"
                            f"  [white]Мудрость: [cyan]{item.wisdom}[/]\n"
                            f"  [white]Выносливость: [cyan]{item.endurance}[/]\n"
                            f"  [white]Здоровье: [cyan]{item.max_hp}[/]\n"
                            f"  [white]Мана: [cyan]{item.max_mp}[/]\n"
                            f"  [white]Защита: [cyan]{item.defense}[/]\n"
                            f"  [white]Атака: [cyan]{item.attack}[/]"
                        )
                    else:
                        console.print(f"  [blue]{item.description}[/]")
                ...
                if use_slot.empty:
                    ...
                    game.location.drop.append((use_slot.itemid, use_slot.count))
                    use_slot.itemid = -1

    @action("settings", "Настройки")
    def command_settings(game: Game):
        for i, (k, v) in enumerate(game.settings.data.items(), 1):
            game.console.print(f"[bold green]{i}[/] [white]{k:<10}[/] [blue]{v}[/]")
        while True:
            game.console.print("[bold green]Выберите настройку (e-exit)[/]")
            select = game.console.prompt(
                2,
                checker=lambda x: x == "e"
                or x.isdigit()
                and int(x) in range(1, len(game.settings.data) + 1),
            )
            if select == "e":
                break
            key = list(game.settings.data.keys())[int(select) - 1]
            game.console.print(
                f"[bold green]Введите значение (t1y/f0n)[/] [white]{key}[/]"
            )
            value = game.console.confirm(2)
            game.settings.set(key, value)
            game.console.print(f"[bold yellow]Значение {key} изменено на {value}[/]")

    @action("stat", "Статистика")
    def command_stat(game: Game):
        ts = game.statistic.ts
        formated = datetime.datetime.fromtimestamp(ts).strftime("%d.%m.%Y %H:%M:%S")
        game.console.print(f"Дата начала: [white]{formated}[/]")
        game.console.print(f"[bold green]Статистика[/]")
        for k, v in game.statistic.counters.items():
            game.console.print(f"  [green]{k}[/] [white]{v}[/]")

    @action("diary", "Показать дневник", "Информация")
    def command_diary(game: Game):
        game.console.print(f"[bold green]Активные квесты[/]")
        if not game.quest_manager.active:
            game.console.print(f"  [white]Нет активных квестов[/]")
        for i, quest in enumerate(game.quest_manager.active, 1):
            game.console.print(f"  [green]{i}[/] [white]{quest.name}[/]")
            game.console.print(f"    [blue]{quest.description}[/]")

    @action("map", "Показать дерево локаций", "Информация")
    def command_map(game: Game):
        location = game.world.root
        visited = set()

        def print_location(location, level=0):
            if location in visited:
                return
            visited.add(location)
            game.console.print(f"{'  ' * level}[green]{location.name}[/]")
            for child in location.sublocations:
                print_location(child, level + 1)
...
class GameMap:
    def set_root(self, root: Location):
        self.root = root

    def save(self):
        data = {}
        for l in self.locations:
            data[l.identifier] = l.save()
        return data

    def load(self, data):
        for l in self.locations:
            l.load(data[l.identifier])

    def get(self, identifier: str) -> Location:
        for l in self.locations:
    ...
    def locations(self) -> set[Location]:
        ...
        while queue:
            ...
            if location in visited:
                continue
            visited.add(location)
            queue.extend(location.sublocations)
        return visited
    ...
    def paths(self):
        ...
        def get_paths(location: Location):
            visited.add(location)
            for sub in location.sublocations:
                if sub not in visited:
                    yield (location, sub)
                    yield from get_paths(sub)

        matrix = {}
        for path in get_paths(self.root):
            matrix.setdefault(path[0], []).append(path[1])
        return matrix


class Statistic(ReqHandler):
    def save(self):
        return [self.ts, list(self.counters.values())]

    def load(self, data):
        self.ts = data[0]
        for i, k in enumerate(data[1]):
            self.counters[list(self.counters.keys())[i]] = k

    def __init__(self):
        self.ts = int(time.time())
        self.counters = {
            "Исполнено команд": 0,
            "Получено предметов": 0,
            "Перемещений": 0,
            "Убито монстров": 0,
        }
    ...
    def __call__(self, g: Game):
        super().__call__(g)
        event_map = {
            "command": lambda e: self.counter("Исполнено команд"),
            "new_drop": lambda e: self.counter("Получено предметов"),
            "location_move": lambda e: self.counter("Перемещений"),
            "entity_killed": lambda e: self.counter("Убито монстров"),
        }
        for e, f in event_map.items():
            self.handler.listen(e)(f)
        return self
...
class Quest:
    def __init__(self, identifier, name, description, requirements, reward=None):
        self.identifier = identifier
        self.name = name
        self.description = description
    ...
    def reset(self):
        self.checked = []  # index of done requirements
    ...
    @property
    def done(self):
        return len(self.checked) == len(self.requirements)

    def check(self, event):

        for i, requirement in enumerate(self.requirements):
            reqtype, *args = shlex.split(requirement)
            # Проверяем тип требования и тип события
            if reqtype == "location_join" and event.type == "location_move":
                # Если это нужное требование и нужное событие
                if args[0] == event.next.identifier:
                    self.checked.append(i)

    def __repr__(self):
        return f"<Quest {self.name!r}>"


class QuestManager(ReqHandler):
    def __init__(self):
        self.active: list[Quest] = []
        self.quests: list[Quest] = []

    def save(self):
        return {q.identifier: q.save() for q in self.active}

    def load(self, data):
        for q, data in data.items():
            self.give(q).load(data)

    def give(self, id) -> (Quest | None):
    ...
    def complete(self, quest: Quest):
        for item in quest.reward:
            type, *args = shlex.split(item)
            if type == "trigger":
                name = args[0]
                scenario = self._game.bestiary.get_scenario(name)
                scenario.run()

    def load_quests(self, *quests: list[Quest]):
...
class BaseMod:
    ...
    def load(self):
        game = self.game
        game.parser.full_load(
            game,
            """
@location "myroom" "Комната" "Тут вы можете отдохнуть"
    drop sword 1
    drop shield 1
    drop hp_bottle 2
    @action "save" "Сохраниться" 0 "save" "none"
        :none
        save
        :!
    @!
    @action "load" "Загрузиться" 0 "load" "none"
        :none
        load
        :!
    @!
    @action "upgrade" "Повысить характеристики" 0 "upgrade" "none"
        :none
        upgrade
        :!
    @!
    @action "rest" "Отдохнуть" 420 "rest" "none"
        :none
        exec game.player.heal(game.player.max_hp * 0.4)
        cast [green]Вы отдохнули.[/]
        :!
    @!
@!

@location "tavern" "Таверна" "Весьма оживленно для такой маленькой деревни"
    npc edward
@!

@location "village" "Деревня" "Захудало и весьма тихо. Но тут достаточно безопасно"
    npc trader
@!

@location "swamp" "Болото" "Здесь где то обитает Руфус"
@!

@location "forest" "Лес" "Тут можно найти монстров и добыть ресурсы."
    npc john
    npc sam
    
    # cmd desc time sc_id sc_state
    @action "explore" "Исследовать" 30 "forest" "none"
        :none
        set chances "[0.45, 0.20, 0.30, 0.05] if game.clock.in_range(23, 7) else [0.55, 0.10, 0, 0.35]"
        set select "random.choices(range(4), chances, k=1)[0]"
        cases "select==0" nothing \
              "select==1" money \
              "select==2" entity \
              "select==3" berry 
        :nothing
            cast [bold red]Ничего не найдено[/]
            exec game.clock.wait(30)
        :!
        :money
            set money "random.randint(1, 15)"
            exec game.player.add_money(money)
            cast [bold green]Вы нашли $money монет[/]
            cast [bold blue]У вас $(game.player.money) монет[/]
        :!
        :entity
            set entity "game.bestiary.get_random_entity()"
            set res "game.battle_manager.new(entity)"
            cases "res" true
            :true
                cast [bold green]Вы победили[/]
                exec game.player.add_money(entity.money)
                cast [bold green]Вы победили $(entity.name) и получили $(entity.money) монет[/]
                cast [bold blue]У вас $(game.player.money) монет[/]
            :!
        :!
        :berry
            cast [bold green]Вы нашли ягоду[/]
            exec game.player.inventory.pickup(game.bestiary, "berry", 1)
        :!
    @!
@!

@location "wall" "Туман" "Не думаю, что смогу отсюда выбраться"
    @action "explore" "Исследовать" 30 "wall" "none"
        :none
        cast Я иду сквозь туман, желая уйти отсюда. Но туман не даёт мне этого сделать
        cast Продвигаясь сквозь туман, я теряю видимость вокруг себя
        cast Но спустя некоторое время я просто возвращаюсь в ту же точку, из которой начал
        :!
    @!
@!
...
@quest "first_quest" "Зайти в лес" "Вы должны найти своих 'спасителей'"
 req "location_join forest"
 rew "trigger first_meeting"
@!

@entities
    # name, s, d, w, e, free, money
    add "Безумная крыса"            1 1 1 1 0 25
    add "Полуночный призрак мага"   1 1 2 2 0 25
    add "Волк"                      1 1 2 1 0 25
    add "Безумец"                   1 1 2 1 1 21
@!

# name desc state identifier known [color] [scenario]
@npc "Торговец" "Продает и покупает интересные вещи" "none" "trader" True blue
    :none
    say Привет, путник, чего желаешь?
    menu '' 'Я хочу кое-что продать' sell \
    'Я хочу купить зелье' buy \
    'Пока, мне пора' bye
        :sell
        sell
        :!
        :buy
        buy hp_bottle mp_bottle knowledge_book
        :!
    :!
@!

@npc "Эдвард" "Хозяин таверны в деревне" "none" "edward"
    :none
    sayb Привет
    say Привет, я пока занят
    end none
    :!
@!

@npc "Джон" "Защитник деревни и друг Сэма" "none" "john"
    :none
    sayb Привет
    say Привет, я пока занят
    end none
...
@npc "Сэм" "Друг Джона, так же защитник деревни" "none" "sam"
    :none
    sayb Привет
    say Привет, я пока занят
    end none
...
@scenario "game" "none" 
    :none
    cast Я покинул свою деревню год или два назад. 
    cast Мной было решено, что мне нужно двигаться дальше и искать себя в этом мире. Я не знаю, куда я пойду. 
    cast Закупив немного еды в дорогу, я покинул очередной маленький закуток. Люди тут добрые. Но я не захотел оставаться и отправился в дальний путь. 
    cast Дорога предстояла дальняя. Мной преодолевались огромные поля, реки и леса. 
    cast Мое чутье мне подсказывало, что погода скоро испортится. Издалека был слышен грохот грома. Я был прав.
    cast Вместе с этим начало темнеть, уже вечер. День прошел для меня быстро, неожиданно быстро. Заблудиться было очень просто. 
    cast Укрыться было негде. Небо затянулось тучами и полил дождь, я пытался бежать от него, но он настиг меня, и в конце концов я весь промок.
    cast Земля под ногами начала расплываться, не было понятно куда я иду. Видимость почти нулевая. Я шел, куда глаза глядят. 
    cast Выйдя из леса через некоторое время я увидел огни. Это были огни деревни.
    sayb Ну наконец то!

    cast Я шел в ее сторону. Дул сильный ветер, деревья качались из стороны в сторону, я еле сдерживал дрожь. 
    cast Прошло несколько минут, огоньки становились ближе. До меня долетел запах дыма. 
    cast Потом я резко почувствовал какой то удар по голове. Я не успел понять, как потерял сознание. 
    cast Последнее, что я услышал, это "Дебил, ты что творишь!?"
    cast ...
    cast ...
    cast ...
    cast ...
    cast ...
    cast Когда я пришел в себя, я очутился в кровати. Тут было весьма тепло и уютно. 
    cast В комнате никого не было, за окном только начинало светлеть. 
    cast [yellow][!][/] Точное время можно узнать командой [green]time[/]. Во-первых вы можете узнать какой день вы пребываете тут, а второе - узнать время. В разное время суток могут происходить разные события, учитывайте это!

    cast Голова жутко болела, было трудно сконцентрироваться на цели. 
    cast Дверь в комнату открылась, в нее вошел незнакомый мне парень.
    saye edward О, значит ты уже пришел в себя
    saye edward Как ты себя чувствуешь? Ничего не болит? 
    sayb Все вроде в полном порядке, но голова жутко болит
    saye edward Оу, да... Прости их за такое недоразумение... Они не специально
    cast Я вопросительно посмотрел на него, ожидая подробностей
    saye edward Если честно давно не было гостей. В плане, наш город окружен мистическим туманом и подвержен постоянным нападениям монстров. 
    saye edward Тут живет всего лишь пара человек, на самом деле. Вообщем я принес тебе немного еды
    give beef 1
    cast [yellow][!][/] Вы получили жаренное мясо. Вы можете узнавать ваше состояние командой [green]me[/]. 
    cast [yellow][!][/] Что бы открыть сумку используйте команду [green]inventory[/]. Там вы можете просмотреть содержимое командой [yellow]v[/], использовать предмет [yellow]u <# ячейки>[/]. После этого вы сможете выйти оттуда командой [yellow]e[/]. 
    cast [yellow][!][/] Забыли названия команд? [green]help[/]. А если нужна справка по игре - [green]guide[/]
    saye edward А... Я совсем забыл забыл представиться, я Эдвард и я хозяин этой таверны. Эта комната твоя на неделю.
    e edward known
    cast [yellow][!][/] В этой комнате вы можете сохранить и загрузить игру командами [green]save[/] и [green]load[/]
    saye edward Своих "спасителей" сможешь встретить в городе и поговорить с ними. Тебе что-то еще нужно?

    menu_exhaust "" "Что за туман?" menu1 \
    "Где найти тех ребят?" menu2 \
    "Но мне надо покинуть деревню, двигаться дальше!" menu3 \
    "Я могу чем то помочь?" menu4

        :menu1
        saye edward Ну... Я на самом деле не смогу полноценно ответить на твой вопросы. 
        saye edward Я лишь могу сказать, что он тут весьма давно. 
        saye edward Никто не может покинуть его предметы, деревня заперта в такой клетке. 
        :!

        :menu2
        saye edward Ты можешь найти их где то в лесу. Они как обычно охотятся на монстров.
        saye edward В последнее время они стали значительно чаще нападать, раньше они не были такими агрессивными.
        saye edward Кто-то говорит, что в этом замешан туман...
        :!

        :menu3
        saye edward Я не держу насильно никого, но это делает туман. 
        saye edward Ты можешь попытаться уйти отсюда, но скорее всего просто вернешься в то же место, откуда шел. 
        saye edward До тебя уже пытались это сделать. Но я готов предложить тебе уютную комнату, могу даже бесплатно, если будешь помогать деревне.
        saye edward Таверна - самое живое место тут. Именно тут ты и встретишь разных жителей деревни. 
        saye edward Может быть ты узнаешь от них что нибудь еще полезное.
        :!

        :menu4
        saye edward Оу, я даже не знаю. Я, как посмотрю, ты искатель приключений, так что в целом ты можешь помочь не только мне, но и всей деревне, защищая ее от монстров.
        saye edward Ну и может стоит поспрашивать других жителей деревни, может ты найдешь какие то еще интересности.
        saye edward А хотя... Все таки дай подумать, все же есть небольшое поручение, у меня закончились ягоды для морса.
        saye edward Можешь найти их в лесу. Днем там безопасно.
        :!

    cast [yellow][!][/] Для того, что бы осмотреться и узнать, что и где можно найти - используйте [green]look[/]. Вы получите информацию о людях рядом, доступных локациях и предметах, которые вы можете взять себе.
    cast [yellow][!][/] [green]go[/] - отправиться в другую доступную локацию
    cast [yellow][!][/] [green]talk[/] - поговорить с нпс
    cast [yellow][!][/] [green]pickup[/] - подобрать предмет
    cast [yellow][!][/] [green]diary[/] - Ваш дневник. Там можно посмотреть квесты
    cast [yellow][!][/] [green bold]look[/] - Осмотритесь, возможно обнаружите что-то интересное
    cast [yellow][!][/] [green bold]upgrade[/] - У вас есть нераспределенные очки характеристик. Вы можете распределить их этой командой
    cast [yellow][!][/] [green].?[/] - покажет информацию о макросах. Там можно выключить анимацию текста, 
    cast [yellow][!][/] цветной вывод, поменять тему, отключить нажатие клавиш для продолжения и просмотреть историю команд
    give_quest first_quest
    end none
    :!
@!

@scenario "first_meeting" "none" 
    :none
    cast Некоторое время я плутаю по лесу. Потом я слышу издали голоса и начинаю на них идти
    cast Я вижу 2 силуэта, которые медленно и внимательно блуждают по лесу
    cast Я подходю к ним и один из них замечает меня
    saye john О, привет, ты уже очнулся?
    cast Второй парень видит, что я подошел к ним и тоже замечает меня
    saye sam Оу, привет, хах
    cast Парень говорит неуверенно, будто это он ко мне подошел, а не наоборот
    menu "" "Привет, мне вроде как сказали найти вас" m1 \
    "Мне кажется твой голос знакомым" m2 
        :m1
        saye john Оу, Эдвард? Ну вообщем то мы хотели извиниться
        :!
        :m2
        saye john Возможно. Дело в том, что Сэм и я не очень хорошо разглядели тебя прошлой ночью
        :!    
    cast Он толкает Сэма в плечо
    saye sam А... да. Вообщем то извини, это я тебя палкой ударил этой ночью. Принял тебя за здешнего монстра
    saye sam Меня зовут Сэм, а этот парень - Джон
    e john known
    e sam known
    sayb Все впорядке. Наверное... Вообщем я хочу узнать у вас, что это за туман?
    saye john Ты не первый, кто спрашивает об этом. Мы тоже не знаем, что это такое
    saye sam Выбраться отсюда практически невозможно. Мы пытались, но туман не даёт нам этого сделать
    saye john Возможно тебе стоит обратиться к Руфусу. Он знает больше, чем мы, ты найдешь его на болоте
    cast [yellow bold][!][/] На данный момент сюжет закончился. Вы можете продолжить игру.
    :!
@!


@items
item "sword" "Меч" "Простой меч, будет полезен" 1
    wear 1 0 0 0 0 0 0 0 WEAPON

item "shield" "Щит" "Базовый щит, не лучше картонного" 1
    wear 0 1 0 0 0 0 0 0 SHIELD

item "hp_bottle" "Малое зелье здоровья" "Восстанавливает 10 здоровья" 5
    use 0 0 0 0 10 0 0 0
    price 1 10

item "mp_bottle" "Малое зелье маны" "Восстанавливает 10 маны" 5
    use 0 0 0 0 0 10 0 0
    price 1 10

item "beef" "Жареная говядина" "Вкусно. Позволяет восстановить немного хп" 3
    use 0 0 0 0 5 0 0 0
    price 1 10

item "money_bag" "Мешочек монет" "Содержит 100 монет" 3
    use 0 0 0 0 0 0 0 100

item "knowledge_book" "Книга знаний" "Дает свободный поинт для прокачки" 2
    use 0 0 0 0 0 0 1 0
    price 1 100

item "berry" "Ягода" "Отличный товар как мне кажется" 10
    price 1 3

item "redflower" "Красноцвет" "Красный цветок от когорого исходит тусклое свечение" 10
    price 1 10

item "blueflower" "Синецвет" "Синий цветок от когорого исходит тусклое свечение" 10
    price 1 10
@!
""",
        )
...
class Game:
    ...
    def __init__(self):
        ...
        self.manager.add_submanagers(lambda: self.location)
        self.player = Entity(
            self.bestiary,
            EntityInfo(name="Игрок", s=1, d=1, w=1, e=1, free=3, money=20),
        )(self)
        self.splashes = [
    ...
    def init_debug(self):
        ...
        def t(e):
            ...
            if self.settings.get("debug"):
                self.console.log(f"[yellow]#{v}[magenta] {e}")
    ...
    def save(self):
        ...
        payload = [self.__version__, *[m.save() for m in modules]]

        pickled = msgpack.packb(payload)
        zipped = zlib.compress(pickled, level=9)

        game.console.print("[red]Выберите тип сохранения")
        names = [
            "base161 (рекомендуется)",
            "base64",
            "base85",
            "base93",
            "Braille",
        ...
        for i, a in enumerate(names, 1):
            game.console.print(f"[green] {i}[/] - {a}")
        select = game.console.number_menu(2, len(names))
        encoded = str(select) + self.encryptor.encode(zipped, type=select)
        return encoded

    def load(self, code):
        try:
            zipped = self.encryptor.decode(code[1:], type=int(code[0]))
            pickled = zlib.decompress(zipped)
            # payload = json.loads(pickled)
            payload = msgpack.unpackb(pickled)
            if payload[0] != self.__version__:
                self.console.print("[bold red]Неверная версия сохранения[/]")
                return False
            for save_data, game_module in zip(
                payload[1:],
                (
                    game.player,
                    game.clock,
                ...
                game_module.load(save_data)

        except zlib.error as e:
            self.console.print(f"[bold red]Повреждено сохранение[/] {e}")
            return False
        except Exception as e:
            self.console.print(f"[bold red]Ошибка загрузки: {e}[/]")
            return False
        return True

    def preload(self):
        self.state = State.RUNNING
        self.console.print("[bold yellow]Добро пожаловать в игру!")
        self.console.print("[green]Вы желаете загрузить сохранение? [gray](y/n)[/]")
        confirm = self.console.confirm(1)
        if confirm:
            while True:
                self.console.print(
                    "[bold green]Введите свой код сохранения (пустой ввод - выход)[/]"
                )
                answer = input()
                if not answer:
                ...
                if self.load(answer):
                    ...
                    self.console.print("[bold red]Не удалось загрузить сохранение")

        else:
            self.console.print("[bold green]Как вас зовут?")
            answer = self.console.prompt(2)
            self.player.name = answer
            clrs = [
                "[bold magenta]",
                "[bold red]",
                "[bold blue]",
            ...
            self.console.print(
                "[bold yellow]Совет: [green]для быстрого пропуска текста выключите анимацию и подтверждение ([gray].f .w[green])"
            )
            while True:
                self.console.print("[bold green]Готовы пройти обучение? [gray](y/n)")
                answer = self.console.confirm(1)
                if answer:
                    break

            self.bestiary.get_scenario("game").run()

    def gen_panels(self):
        def gen_text():

            return (
                f"[red] HP {self.player.hp:.2f}/{self.player.max_hp:.2f}[/] | "
                f"[cyan]MP {self.player.mp:.2f}/{self.player.max_mp:.2f}[/] | "
                f"[green]FREE: {self.player.free_points}[/] | "
                f"[yellow]БАЛАНС {self.player.money}[/] | "
                f"[blue]ЛОКАЦИЯ: {self.location.name}[/] | "
                f"[red]ПРЕДМЕТЫ И ЛЮДИ: {len(self.location.npcs)}-{len(self.location.drop)}[/] |"
                f"[magenta]ДЕНЬ {self.clock.days}[/] | "
                f"[cyan]КВЕСТЫ: {len(self.quest_manager.active)}[/]"
            )
...
if __name__ == "__main__":
    ...
    try:
        ...
        # move left 2 and print 2 spaces
        print("\x1b[2D  \x1b[2D")
        game.console.print("[bold red]Выход из игры...[/]")
