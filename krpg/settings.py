...
class Param:
    def __init__(self, id: str, name: str, description: str, variants: dict[str, str] = None):
        self.id = id
        self.name = name
...
class Settings:
    ...
    @param('debug', 'Отладка', 'Включает или отключает вывод отладки', {'enable': 'Включить', 'disable': 'Выключить'})
    ...
    def settings_command(game: Game):
        ...
        view = lambda x: f"[green]{x.name}[/] [yellow]\[{x.value if x.value is not None else 'По умолчанию'}][/] - {x.description}" if isinstance(x, Param) else "-"
        while True:
            param: Param = game.console.menu(2, variants, 'e', view, title="Выберете параметр для изменения")
            if not param:
                return
            if param.variants:
                new_value = game.console.menu(3, list(param.variants.items()), 'e', lambda x: x[1])
                if not new_value:
                    continue
                ...
                if new_value == 'e':
                    continue
            game.events.dispatch(Events.SETTING_CHANGE, setting=param.id, value=new_value)
