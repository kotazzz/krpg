...
class Game:
    ...
    def execute(self, command: Command):
        ...
        c, a, t, ft = command.command, command.args, command.text[len(command.command):], command.text
        if c == "print":
            self.print(t)
        elif c == "exec":
            ...
            self.scenario["data":"one":"section"]["location":"one":"command"].args[0] = a[0]
    ...
    def main(self):
        ...
        location = self.scenario["gamemap":"one":"section"]["spawn":"one":"command"].args[0]
        self.scenario.body.append(Section("data", [
            Command(f"location {location}"),
            Command(f"state RUN")
            ]))
        
        get_state = lambda: self.scenario["data":"one":"section"]["state":"one":"command"].args[0]
        get_location = lambda: self.scenario["data":"one":"section"]["location":"one":"command"].args[0]
